...
The sample average of μ̂ across tests for which the null is rejected is not approximately equal to the true value of μ, **especially as the effect size is smaller and power is lower**.
```{r}
mean_plot1 =
  sim_df2 %>% 
  group_by(mean) %>%
...
Here are plots that depict similar information as above but with all μ̂ represented, not just the average estimate of μ̂.
```{r}
mean_plot3 =
  sim_df2 %>% 
  ggplot(
