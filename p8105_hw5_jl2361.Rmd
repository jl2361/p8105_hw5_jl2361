...
## Using prop.test
We then create a `myproptest` function to apply the prop.test function to estimate the proportio of homicides that are unsolved, apply the broom::tidy, and save the output (estimated proportion and upper and lower bounds of the 95% CI) as a tibble. We use `myproptest` for the city of Baltimore, MD and note that the filtered dataframe needs to be converted to a list of vectors prior to applying `myproptest`.
```{r}
...
We also create a `sim_ttest` function to map to `sim_df` and save μ̂  and the p-value arising from a test of H:μ=0 using α=0.05
```{r}
...
## Plots
Here is a plot showing the proportion of times the null was rejected (the power of the test) on the y axis and the true value of μ on the x axis. As the effect size gets bigger, we see that power increases as well. 
...
Next, here is a plot showing the average estimate of μ̂  on the y axis and the true value of μ on the x axis. There is also a second plot (right) showing the average estimate of μ̂  only in samples for which the null was rejected on the y axis and the true value of μ on the x axis. 
